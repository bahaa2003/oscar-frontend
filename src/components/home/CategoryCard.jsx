...
const CategoryCard = ({
  ...
  if (isPlain) {
    ...
    return (
      <button
        ...
        className="storefront-category-card storefront-category-card--plain group relative isolate flex h-full w-full origin-center select-none flex-col rounded-[1.4rem] text-start transition-all duration-200 ease-out hover:-translate-y-0.5 hover:scale-[1.002]"
  ...
  return (
    <button
      ...
      onClick={() => onSelect(category.id)}
      className={cn(
        'storefront-category-card storefront-category-card--neon group relative flex h-full flex-col overflow-visible rounded-[1.9rem] border border-transparent bg-transparent text-start shadow-none transition-all hover:-translate-y-0.5 hover:scale-[1.002]',
