...
const getBillingModeLabel = (value, tx) => (
  normalizeBillingMode(value) === 'quantity_only'
    ? tx('كوتا كمية فقط', 'Quantity only')
    : tx('قياسي', 'Standard')
);
  ...
  const openEditModal = (group = null) => {
    if (group) {
      ...
      setFormData({
        ...
        billingMode: normalizeBillingMode(group.billingMode),
      });
    } else {
      resetForm();
    }
    ...
    try {
      if (editingGroup) {
        await updateGroup(editingGroup.id, {
          ...
          billingMode: normalizeBillingMode(formData.billingMode),
        });
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-center">{tx('نظام الفوترة', 'Billing mode')}</TableHead>
            ...
            {groups.map((group) => {
              ...
              return (
                <TableRow key={group.id}>
                  ...
                  <TableCell className="text-center">
                    <span className="inline-flex min-w-[96px] items-center justify-center rounded-full border border-[color:rgb(var(--color-border-rgb)/0.9)] bg-[color:rgb(var(--color-surface-rgb)/0.82)] px-3 py-1 text-xs font-semibold text-[var(--color-text-secondary)]">
                      {getBillingModeLabel(group.billingMode, tx)}
                    </span>
                  </TableCell>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <label className="space-y-2">
              <span className="block text-sm font-semibold text-[var(--color-text)]">
                {tx('نظام الفوترة', 'Billing mode')}
              </span>
              <select
                className={selectClassName}
                value={formData.billingMode}
                onChange={(event) => setFormData((current) => ({ ...current, billingMode: normalizeBillingMode(event.target.value) }))}
              >
                <option value="standard">{tx('قياسي', 'Standard')}</option>
                <option value="quantity_only">{tx('كوتا كمية فقط', 'Quantity only')}</option>
              </select>
            </label>
          </div>
