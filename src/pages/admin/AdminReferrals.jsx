...
const AdminReferrals = () => {
  ...
  const realOwners = useMemo(() => (Array.isArray(users) ? users : [])
    .map(normalizeOwner)
    .filter((owner) => owner.referrals.length || owner.earnings || owner.withdrawals.length), [users]);
  ...
  const addWithdrawalMethod = () => {
    ...
    if (!name) return;
    persistMethods([...withdrawalMethods, { id: `method-${Date.now()}`, name, enabled: true, requiresAccount: true, discountPercent: 0 }]);
    setNewMethodName('');
