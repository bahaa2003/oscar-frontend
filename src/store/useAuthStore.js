...
const pickPersistedUser = (user) => {
  ...
  const rawGroup = (
    user.group && typeof user.group === 'object'
      ? user.group
      : user.groupId && typeof user.groupId === 'object'
        ? user.groupId
        : (user.group || user.groupId || null)
  );
  ...
  return {
    ...
    billingMode: normalizeBillingMode(user.billingMode || (typeof rawGroup === 'object' ? rawGroup?.billingMode : '')),
    phone: user.phone || '',
...
const useAuthStore = create((set, get) => ({
    ...
    profileLastLoadedAt: persistedAuthState.profileLastLoadedAt || 0,
      ...
      loginWithGoogle: async (options = {}) => {
        ...
        try {
          const response = await apiClient.auth.loginWithGoogle(options);
