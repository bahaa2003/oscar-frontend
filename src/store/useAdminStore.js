...
const useAdminStore = create((set, get) => ({
      ...
      updateQuantityLimit: async (userId, quantityLimit, actor = null) => {
        const normalizedQuantityLimit = Math.max(0, toFiniteNumber(quantityLimit, 0));

        const updatedUser = apiClient.users.updateQuantityLimit
          ? await apiClient.users.updateQuantityLimit(userId, normalizedQuantityLimit, actor)
          : await apiClient.users.updateProfile(userId, { quantityLimit: normalizedQuantityLimit }, actor);
            ...
            entry.id === userId
              ? {
                ...
                quantityLimit: Math.max(
                  0,
                  toFiniteNumber(updatedUser?.quantityLimit, normalizedQuantityLimit)
                ),
              }
              : entry
          )),
        ...
        get().appendAdminActivity({
          ...
          entityId: userId,
          status: 'updated',
          source: 'quantity_billing',
        });

        return updatedUser;
      },

      resetQuantity: async (userId, actor = null) => {
        const updatedUser = apiClient.users.resetQuantity
          ? await apiClient.users.resetQuantity(userId, actor)
          : await apiClient.users.updateProfile(userId, { quantityUsed: 0 }, actor);
          ...
          users: state.users.map((entry) => (
            entry.id === userId
              ? {
                ...entry,
                ...(updatedUser || {}),
                quantityUsed: toFiniteNumber(updatedUser?.quantityUsed, 0),
              }
              : entry
          )),
          usersLastLoadedAt: Date.now(),
